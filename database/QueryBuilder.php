...
class QueryBuilder
{
	...
	public function updateAll($table, $cols,$values,$uniq)
	{
		...
		for ($i=0; $i < $countCol; $i++) { 
			if($i != $countCol-1){
				$sql = $sql."{$cols[$i]} = '{$values[$i]}',";
			}else{
				$sql = $sql."{$cols[$i]} = '{$values[$i]}'";
			}
		}

		$sql = $sql." WHERE id = {$uniq}";
		$query = $this->pdo->prepare($sql);
	...
	public function logView($id)
	{
		...
		if($find){			
			$select = $this->pdo->prepare("select * from businesses_views where company_id = {$id}");
		...
		$query = $this->pdo->prepare(
			"SELECT  category_id, name
				FROM companies_categories
   				...
   				ON categories.id = companies_categories.category_id
   				WHERE company_id = {$id}"
		);
		$query->execute();
